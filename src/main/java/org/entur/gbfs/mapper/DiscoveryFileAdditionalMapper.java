...
public class DiscoveryFileAdditionalMapper {
    ...
    public org.entur.gbfs.v3_0.gbfs.GBFSData map(Map<String, GBFSFeeds> source, @Context String sourceLanguage) {
        List<GBFSFeed> mappedFeeds = source.get(sourceLanguage).getFeeds().stream()
                .filter(this::filterLegacySourceFeeds)
                .map(this::map)
                .toList();

        return new org.entur.gbfs.v3_0.gbfs.GBFSData()
                .withFeeds(mappedFeeds);
    }

    public Map<String, org.entur.gbfs.v2_3.gbfs.GBFSFeeds> map(org.entur.gbfs.v3_0.gbfs.GBFSData source, @Context String targetLanguage) {
    ...
    private org.entur.gbfs.v3_0.gbfs.GBFSFeed map(org.entur.gbfs.v2_3.gbfs.GBFSFeed sourceFeed) {
    ...
    private org.entur.gbfs.v2_3.gbfs.GBFSFeed map(org.entur.gbfs.v3_0.gbfs.GBFSFeed sourceFeed) {
